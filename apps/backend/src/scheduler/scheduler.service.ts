...
export class SchedulerService {
  ...
  @Cron('0 7 * * 1', { name: 'weekly_contribution_debit', timeZone: 'Africa/Lagos' })
  async runWeeklyContributionDebit(): Promise<void> {
    ...
    try {
      ...
      const members = await this.prisma.member.findMany({
        ...
        include: {
          association: { select: { id: true, name: true, plan: true, poolBalance: true } },
        },
      });
      ...
      for (const member of members) {
        ...
        if (existing) {
          this.logger.debug(`Member ${member.id} already processed for week ${monday.toISOString().slice(0, 10)}`);
          continue;
        }
        ...
        try {
          ...
          debitResult = { success: false, responseCode: 'ERROR', responseMessage: err?.message ?? 'ISW unreachable' };
        }
  ...
  async triggerManualDebit(): Promise<{ message: string }> {
    ...
    return { message: 'Weekly debit triggered manually — check logs for progress' };
  }
