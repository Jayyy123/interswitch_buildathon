...
export class MembersService {
  ...
  async enrollMembers(
    ...
    if (newMemberIds.length > 0) {
      void Promise.resolve().then(() =>
        this._provisionWalletsInBackground(
          newMemberIds,
          association.name,
          association.plan,
          weeklyAmount,
          ...
          this.logger.error(
            'Background wallet provisioning crashed',
            err?.message,
          ),
        ),
      );
  ...
  async retryWallet(
    ...
    void Promise.resolve().then(() =>
      this._provisionWalletForMember(
        memberId,
        association.name,
        association.plan,
        weeklyAmount,
      ).catch((err) =>
        this.logger.error(`Retry wallet failed for ${memberId}`, err?.message),
      ),
    );
